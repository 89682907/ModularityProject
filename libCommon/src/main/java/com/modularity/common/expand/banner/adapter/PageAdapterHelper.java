...
public class PageAdapterHelper {
    ...
    public void onCreateViewHolder(ViewGroup parent, View itemView) {
        ...
        lp.width = parent.getWidth() - SizeManager.dp2px(2 * (sPagePadding + sShowLeftCardWidth));
        itemView.setLayoutParams(lp);
    }

    public void onBindViewHolder(View itemView, final int position, int itemCount) {
        int padding = SizeManager.dp2px(sPagePadding);
        itemView.setPadding(padding, 0, padding, 0);
        int leftMarin = position == 0 ? padding + SizeManager.dp2px(sShowLeftCardWidth) : 0;
        int rightMarin = position == itemCount - 1 ? padding + SizeManager.dp2px(sShowLeftCardWidth) : 0;
