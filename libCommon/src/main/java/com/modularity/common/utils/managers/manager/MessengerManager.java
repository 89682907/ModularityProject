...
public class MessengerManager {
    ...
    public static void register() {
        if (ManagerBridge.isMainProcess()) {
            if (ManagerBridge.isServiceRunning(ServerService.class.getName())) {
                Log.i("MessengerUtils", "Server service is running.");
                return;
            }
            Intent intent = new Intent(Managers.getApp(), ServerService.class);
            Managers.getApp().startService(intent);
            return;
        }
    ...
    public static void unregister() {
        if (ManagerBridge.isMainProcess()) {
            ...
            Intent intent = new Intent(Managers.getApp(), ServerService.class);
            Managers.getApp().stopService(intent);
        }
        if (sLocalClient != null) {
            sLocalClient.unbind();
        }
    ...
    static class Client {
        ...
        Handler mReceiveServeMsgHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                data.setClassLoader(MessengerManager.class.getClassLoader());
        ...
        ServiceConnection mConn   = new ServiceConnection() {
            ...
            public void onServiceConnected(ComponentName name, IBinder service) {
                ...
                int key = ManagerBridge.getCurrentProcessName().hashCode();
                Message msg = Message.obtain(mReceiveServeMsgHandler, WHAT_SUBSCRIBE, key, 0);
                msg.getData().setClassLoader(MessengerManager.class.getClassLoader());
                msg.replyTo = mClient;
                try {
                    mServer.send(msg);
        ...
        boolean bind() {
            if (TextUtils.isEmpty(mPkgName)) {
                Intent intent = new Intent(Managers.getApp(), ServerService.class);
                return Managers.getApp().bindService(intent, mConn, Context.BIND_AUTO_CREATE);
            }
            if (ManagerBridge.isAppInstalled(mPkgName)) {
                if (ManagerBridge.isAppRunning(mPkgName)) {
        ...
        void unbind() {
            ...
            try {
                Managers.getApp().unbindService(mConn);
        ...
        private boolean send2Server(Bundle bundle) {
            ...
            bundle.setClassLoader(MessengerManager.class.getClassLoader());
            msg.setData(bundle);
            msg.replyTo = mClient;
            try {
                mServer.send(msg);
