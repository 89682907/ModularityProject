...
public final class CacheDoubleUtils implements CacheConstants {
    ...
    private final CacheMemoryManager mCacheMemoryManager;
    private final CacheDiskManager   mCacheDiskManager;
    ...
    public static CacheDoubleUtils getInstance() {
        return getInstance(CacheMemoryManager.getInstance(), CacheDiskManager.getInstance());
    }
     ...
     * @return the single {@link CacheDoubleUtils} instance
     */
    public static CacheDoubleUtils getInstance(@NonNull final CacheMemoryManager cacheMemoryManager,
                                               @NonNull final CacheDiskManager cacheDiskManager) {
        final String cacheKey = cacheDiskManager.toString() + "_" + cacheMemoryManager.toString();
        CacheDoubleUtils cache = CACHE_MAP.get(cacheKey);
        if (cache == null) {
            synchronized (CacheDoubleUtils.class) {
                cache = CACHE_MAP.get(cacheKey);
                if (cache == null) {
                    cache = new CacheDoubleUtils(cacheMemoryManager, cacheDiskManager);
    ...
    private CacheDoubleUtils(CacheMemoryManager cacheMemoryManager, CacheDiskManager cacheUtils) {
        mCacheMemoryManager = cacheMemoryManager;
        mCacheDiskManager = cacheUtils;
    }
    ...
    public void put(@NonNull final String key, byte[] value, final int saveTime) {
        mCacheMemoryManager.put(key, value, saveTime);
        mCacheDiskManager.put(key, value, saveTime);
    }
    ...
    public byte[] getBytes(@NonNull final String key, final byte[] defaultValue) {
        byte[] obj = mCacheMemoryManager.get(key);
        if (obj != null) return obj;
        return mCacheDiskManager.getBytes(key, defaultValue);
    }
    ...
    public String getString(@NonNull final String key, final String defaultValue) {
        String obj = mCacheMemoryManager.get(key);
        if (obj != null) return obj;
        return mCacheDiskManager.getString(key, defaultValue);
    }
    ...
    public JSONObject getJSONObject(@NonNull final String key, final JSONObject defaultValue) {
        JSONObject obj = mCacheMemoryManager.get(key);
        if (obj != null) return obj;
        return mCacheDiskManager.getJSONObject(key, defaultValue);
    }
    ...
    public JSONArray getJSONArray(@NonNull final String key, final JSONArray defaultValue) {
        JSONArray obj = mCacheMemoryManager.get(key);
        if (obj != null) return obj;
        return mCacheDiskManager.getJSONArray(key, defaultValue);
    }
    ...
    public Bitmap getBitmap(@NonNull final String key, final Bitmap defaultValue) {
        Bitmap obj = mCacheMemoryManager.get(key);
        if (obj != null) return obj;
        return mCacheDiskManager.getBitmap(key, defaultValue);
    }
    ...
    public Drawable getDrawable(@NonNull final String key, final Drawable defaultValue) {
        Drawable obj = mCacheMemoryManager.get(key);
        if (obj != null) return obj;
        return mCacheDiskManager.getDrawable(key, defaultValue);
    }
    ...
    public <T> T getParcelable(@NonNull final String key,
        ...
        T value = mCacheMemoryManager.get(key);
        if (value != null) return value;
        return mCacheDiskManager.getParcelable(key, creator, defaultValue);
    }
    ...
    public Object getSerializable(@NonNull final String key, final Object defaultValue) {
        Object obj = mCacheMemoryManager.get(key);
        if (obj != null) return obj;
        return mCacheDiskManager.getSerializable(key, defaultValue);
    }
    ...
    public long getCacheDiskSize() {
        return mCacheDiskManager.getCacheSize();
    }
    ...
    public int getCacheDiskCount() {
        return mCacheDiskManager.getCacheCount();
    }
    ...
    public int getCacheMemoryCount() {
        return mCacheMemoryManager.getCacheCount();
    }
    ...
    public void remove(@NonNull String key) {
        mCacheMemoryManager.remove(key);
        mCacheDiskManager.remove(key);
    }
     ...
     */
    public void clear() {
        mCacheMemoryManager.clear();
        mCacheDiskManager.clear();
    }
}
