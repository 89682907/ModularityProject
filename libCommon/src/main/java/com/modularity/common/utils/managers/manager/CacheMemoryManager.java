...
public final class CacheMemoryManager implements CacheConstants {

    private static final int DEFAULT_MAX_COUNT = 256;

    private static final Map<String, CacheMemoryManager> CACHE_MAP = new HashMap<>();

    private final String                       mCacheKey;
    private final LruCache<String, CacheValue> mMemoryCache;

    /**
     * Return the single {@link CacheMemoryManager} instance.
     *
     * @return the single {@link CacheMemoryManager} instance
     */
     ...
     * @return the single {@link CacheMemoryManager} instance
     */
    public static CacheMemoryManager getInstance(final int maxCount) {
     ...
     * @return the single {@link CacheMemoryManager} instance
     */
    public static CacheMemoryManager getInstance(final String cacheKey, final int maxCount) {
        CacheMemoryManager cache = CACHE_MAP.get(cacheKey);
        if (cache == null) {
            synchronized (CacheMemoryManager.class) {
                cache = CACHE_MAP.get(cacheKey);
                if (cache == null) {
                    cache = new CacheMemoryManager(cacheKey, new LruCache<String, CacheValue>(maxCount));
    ...
    private CacheMemoryManager(String cacheKey, LruCache<String, CacheValue> memoryCache) {
        mCacheKey = cacheKey;
