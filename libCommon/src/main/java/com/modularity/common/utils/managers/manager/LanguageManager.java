...
public class LanguageManager {
    ...
    private static void applyLanguage(@NonNull final Locale locale,
                                      ...
                                      final boolean isNeedStartActivity) {
        if (isFollowSystem) {
            ManagerBridge.getSpUtils4Utils().put(KEY_LOCALE, VALUE_FOLLOW_SYSTEM);
        } else {
            ManagerBridge.getSpUtils4Utils().put(KEY_LOCALE, locale2String(locale));
        }

        updateLanguage(Managers.getApp(), locale);

        if (isNeedStartActivity) {
            Intent intent = new Intent();
            String realActivityClassName = TextUtils.isEmpty(activityClassName) ? ManagerBridge.getLauncherActivity() : activityClassName;
            intent.setComponent(new ComponentName(Managers.getApp(), realActivityClassName));
    ...
    /**
     * Return whether applied the system language by {@link LanguageManager}.
     *
     * @return {@code true}: yes<br>{@code false}: no
     */
    public static boolean isAppliedSystemLanguage() {
        return VALUE_FOLLOW_SYSTEM.equals(ManagerBridge.getSpUtils4Utils().getString(KEY_LOCALE));
    }

    /**
     * Return whether applied the language by {@link LanguageManager}.
     *
     * @return {@code true}: yes<br>{@code false}: no
     */
    public static boolean isAppliedLanguage() {
        return !TextUtils.isEmpty(ManagerBridge.getSpUtils4Utils().getString(KEY_LOCALE));
    }
    ...
    public static boolean isAppliedLanguage(Locale locale) {
        final String spLocale = ManagerBridge.getSpUtils4Utils().getString(KEY_LOCALE);
    ...
    public static Locale getCurrentLocale() {
        return Managers.getApp().getResources().getConfiguration().locale;
    }

    static void applyLanguage(@NonNull final Activity activity) {
        String spLocale = ManagerBridge.getSpUtils4Utils().getString(KEY_LOCALE);
        if (TextUtils.isEmpty(spLocale)) {
            return;
        }
        ...
        if (VALUE_FOLLOW_SYSTEM.equals(spLocale)) {
            ...
            updateLanguage(Managers.getApp(), sysLocale);
        ...
        updateLanguage(Managers.getApp(), settingLocale);
    ...
    private static boolean isSameLocale(Locale l0, Locale l1) {
        return ManagerBridge.equals(l1.getLanguage(), l0.getLanguage())
                && ManagerBridge.equals(l1.getCountry(), l0.getCountry());
    }
}
