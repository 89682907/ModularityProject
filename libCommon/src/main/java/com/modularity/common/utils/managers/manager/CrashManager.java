...
public final class CrashManager {
    ...
    public static void init(final String crashDirPath, final OnCrashListener onCrashListener) {
        String dirPath;
        if (ManagerBridge.isSpace(crashDirPath)) {
            if (ManagerBridge.isSDCardEnableByEnvironment()
                    && Managers.getApp().getExternalFilesDir(null) != null)
                dirPath = Managers.getApp().getExternalFilesDir(null) + FILE_SEP + "crash" + FILE_SEP;
            else {
                dirPath = Managers.getApp().getFilesDir() + FILE_SEP + "crash" + FILE_SEP;
            }
        } else {
        ...
        return new UncaughtExceptionHandler() {
            ...
            public void uncaughtException(@NonNull final Thread t, @NonNull final Throwable e) {
                ...
                final String head = "************* Log Head ****************" +
                        ...
                        "\n************* Log Head ****************\n\n";
                sb.append(head).append(ManagerBridge.getFullStackTrace(e));
                final String crashInfo = sb.toString();
                final String crashFile = dirPath + time + ".txt";
                ManagerBridge.writeFileFromString(crashFile, crashInfo, true);
