...
public final class ToastManager {
    ...
    private static void show(final int resId, final int duration, final Object... args) {
        try {
            CharSequence text = Managers.getApp().getResources().getText(resId);
    ...
    private static IToast newToast() {
        if (NotificationManagerCompat.from(Managers.getApp()).areNotificationsEnabled()) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                if (!ManagerBridge.isGrantedDrawOverlays()) {
                    return new SystemToast(new Toast(Managers.getApp()));
                }
            }
        }
        return new ToastWithoutNotification(new Toast(Managers.getApp()));
    }

    private static View getView(@LayoutRes final int layoutId) {
        LayoutInflater inflate =
                (LayoutInflater) Managers.getApp().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    static class ToastWithoutNotification extends AbsToast {
        ...
        public void show() {
            ...
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N_MR1) {
                mWM = (WindowManager) Managers.getApp().getSystemService(Context.WINDOW_SERVICE);
                mParams.type = WindowManager.LayoutParams.TYPE_TOAST;
            } else if (ManagerBridge.isGrantedDrawOverlays()) {
                mWM = (WindowManager) Managers.getApp().getSystemService(Context.WINDOW_SERVICE);
                ...
                Context topActivityOrApp = ManagerBridge.getTopActivityOrApp();
                ...
                ManagerBridge.addActivityLifecycleCallbacks(topActivity, getActivityLifecycleCallbacks());
            }
        ...
        private void setToastParams() {
            ...
            mParams.packageName = Managers.getApp().getPackageName();
        ...
        private Managers.ActivityLifecycleCallbacks getActivityLifecycleCallbacks() {
            return new Managers.ActivityLifecycleCallbacks() {
                @Override
    ...
    static abstract class AbsToast implements IToast {
        ...
        public void setMsgView(CharSequence text) {
            ...
            if (mToastView == null || mToastView.findViewById(android.R.id.message) == null) {
                mToastView = ToastManager.getView(R.layout.toast_layout);
                mToast.setView(mToastView);
            }
