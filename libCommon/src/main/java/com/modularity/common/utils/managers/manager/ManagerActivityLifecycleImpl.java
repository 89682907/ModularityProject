...
final class ManagerActivityLifecycleImpl implements Application.ActivityLifecycleCallbacks {

    static final ManagerActivityLifecycleImpl INSTANCE = new ManagerActivityLifecycleImpl();

    private final LinkedList<Activity> mActivityList = new LinkedList<>();

    private final List<Managers.OnAppStatusChangedListener>                mStatusListeners               = new ArrayList<>();
    private final Map<Activity, List<Managers.ActivityLifecycleCallbacks>> mActivityLifecycleCallbacksMap = new ConcurrentHashMap<>();
    ...
    Activity getTopActivity() {
        ...
        for (Activity activity : activityList) {
            if (!ManagerBridge.isActivityAlive(activity)) {
                continue;
            }
            return activity;
        }
    ...
    void addOnAppStatusChangedListener(final Managers.OnAppStatusChangedListener listener) {
        mStatusListeners.add(listener);
    }

    void removeOnAppStatusChangedListener(final Managers.OnAppStatusChangedListener listener) {
        mStatusListeners.remove(listener);
    }

    void addActivityLifecycleCallbacks(final Activity activity,
                                       final Managers.ActivityLifecycleCallbacks listener) {
    ...
    private void addActivityLifecycleCallbacksInner(final Activity activity,
                                                    final Managers.ActivityLifecycleCallbacks lifecycleCallbacks) {
        List<Managers.ActivityLifecycleCallbacks> callbacks = mActivityLifecycleCallbacksMap.get(activity);
        if (callbacks == null) {
    ...
    private void consumeActivityLifecycleCallbacks(Activity activity, Lifecycle.Event event) {
        List<Managers.ActivityLifecycleCallbacks> listeners = mActivityLifecycleCallbacksMap.get(activity);
        if (listeners != null) {
            for (Managers.ActivityLifecycleCallbacks listener : listeners) {
    ...
    public void onActivityCreated(@NonNull Activity activity, Bundle savedInstanceState) {
        ManagerBridge.applyLanguage(activity);
        setAnimatorsEnabled();
    ...
    public void onActivityDestroyed(@NonNull Activity activity) {
        ...
        ManagerBridge.fixSoftInputLeaks(activity);
    ...
    private void postStatus(final Activity activity, final boolean isForeground) {
        ...
        for (Managers.OnAppStatusChangedListener statusListener : mStatusListeners) {
            if (isForeground) {
