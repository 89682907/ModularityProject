...
public final class BarManager {
    ...
    public static void setStatusBarColor(@NonNull final View fakeStatusBar,
                                         @ColorInt final int color) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) return;
        Activity activity = ManagerBridge.getActivityByContext(fakeStatusBar.getContext());
    ...
    public static int getActionBarHeight() {
        ...
        if (Managers.getApp().getTheme().resolveAttribute(android.R.attr.actionBarSize, tv, true)) {
    ...
    public static boolean isNavBarVisible(@NonNull final Window window) {
        ...
        if (isVisible) {
            ...
            if (ManagerBridge.isSamsung()
                    && Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1
                    && Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
                try {
                    return Settings.Global.getInt(Managers.getApp().getContentResolver(), "navigationbar_hide_bar_enabled") == 0;
    ...
    private static String getResNameById(int id) {
        try {
            return Managers.getApp().getResources().getResourceEntryName(id);
        } catch (Exception ignore) {
            return "";
        }
    }
    ...
    public static boolean isSupportNavBar() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            WindowManager wm = (WindowManager) Managers.getApp().getSystemService(Context.WINDOW_SERVICE);
        ...
        boolean menu = ViewConfiguration.get(Managers.getApp()).hasPermanentMenuKey();
