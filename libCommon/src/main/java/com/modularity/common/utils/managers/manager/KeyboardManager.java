...
public final class KeyboardManager {
    ...
    public static void showSoftInput() {
        InputMethodManager imm = (InputMethodManager) Managers.getApp().getSystemService(Context.INPUT_METHOD_SERVICE);
        if (imm == null) {
            return;
        }
    ...
    public static void showSoftInput(@NonNull final View view, final int flags) {
        InputMethodManager imm =
                (InputMethodManager) Managers.getApp().getSystemService(Context.INPUT_METHOD_SERVICE);
        if (imm == null) return;
    ...
    public static void hideSoftInputByToggle(final Activity activity) {
        ...
        if (Math.abs(delta) > 500 && KeyboardManager.isSoftInputVisible(activity)) {
    ...
    private static int getDecorViewInvisibleHeight(@NonNull final Window window) {
        ...
        if (delta <= ManagerBridge.getNavBarHeight() + ManagerBridge.getStatusBarHeight()) {
    ...
    private static int getContentViewInvisibleHeight(final Window window) {
        ...
        if (delta <= ManagerBridge.getStatusBarHeight() + ManagerBridge.getNavBarHeight()) {
            return 0;
        }
        return delta;
    }
