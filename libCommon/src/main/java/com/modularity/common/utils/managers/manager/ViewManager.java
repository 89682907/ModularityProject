...
public class ViewManager {
    ...
    public static void runOnUiThread(final Runnable runnable) {
        ManagerBridge.runOnUiThread(runnable);
    }
    ...
    public static void runOnUiThreadDelayed(final Runnable runnable, long delayMillis) {
        ManagerBridge.runOnUiThreadDelayed(runnable, delayMillis);
    }
    ...
    public static boolean isLayoutRtl() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                primaryLocale = Managers.getApp().getResources().getConfiguration().getLocales().get(0);
            } else {
                primaryLocale = Managers.getApp().getResources().getConfiguration().locale;
            }
