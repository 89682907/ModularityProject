...
public class Camera1Manager extends BaseCameraManager<Integer> {
    ...
    public Camera1Manager(CameraPreview cameraPreview) {
        ...
        cameraPreview.setCameraPreviewCallback(new CameraPreviewCallback() {
            ...
            public void onAvailable(CameraPreview cameraPreview) {
                LogManager.iTag(TAG, "onAvailable : " + cameraPreview.isAvailable());
                if (isCameraOpened()) {
                    setupPreview();
                }
            }
    ...
    public void openCamera(final CameraOpenListener cameraOpenListener) {
        ...
        backgroundHandler.post(new Runnable() {
            @Override
            public void run() {
                LogManager.iTag(TAG, "openCamera");
                try {
    ...
    public void switchCamera(int cameraFace) {
        LogManager.iTag(TAG, "switchCamera : " + cameraFace);
    ...
    public float getMaxZoom() {
        if (zoomRatios == null) {
            LogManager.iTag(TAG, "Try to get max zoom while it's not ready.");
            return 0;
        }
        if (maxZoom == 0) {
    ...
    public void setDisplayOrientation(int displayOrientation) {
        LogManager.iTag(TAG, "displayOrientation : " + displayOrientation);
            ...
            public void run() {
                try {
                    if (!takingPicture) {
                        ...
                        LogManager.iTag(TAG, "takePicture : taking picture");
                    }
                } catch (Exception ex) {
                    takingPicture = false;
                    LogManager.iTag(TAG, "takePicture error : " + ex);
    ...
    private void adjustCameraParameters(boolean forceCalculateSizes, boolean changeFocusMode, boolean changeFlashMode) {
        ...
        LogManager.iTag(TAG, "adjustCameraParameters size cost : " + (System.currentTimeMillis() - start) + " ms");

        start = System.currentTimeMillis();
        if (changeFocusMode) {
            setFocusModeInternal(parameters);
        }
        LogManager.iTag(TAG, "adjustCameraParameters focus cost : " + (System.currentTimeMillis() - start) + " ms");

        start = System.currentTimeMillis();
        if (changeFlashMode) {
            setFlashModeInternal(parameters);
        }
        LogManager.iTag(TAG, "adjustCameraParameters flash cost : " + (System.currentTimeMillis() - start) + " ms");

        start = System.currentTimeMillis();
        setZoomInternal(parameters);
        LogManager.iTag(TAG, "adjustCameraParameters zoom cost : " + (System.currentTimeMillis() - start) + " ms");
        ...
        LogManager.iTag(TAG, "adjustCameraParameters restart preview cost : " + (System.currentTimeMillis() - start) + " ms");
    }
    ...
    private void onPictureTakenInternal(byte[] bytes) {
        ...
        try {
            ...
            LogManager.iTag(TAG, "Can't save exif info: " + error.getMessage());
        }
    }
    ...
    private void setAutoFocusInternal(@NonNull Camera.Parameters parameters) {
        try {
            ...
            LogManager.iTag(TAG, "setAutoFocusInternal " + ex);
        }
    }
    ...
    private void setFlashModeInternal(@Nullable Camera.Parameters parameters) {
        ...
        try {
            ...
            LogManager.iTag(TAG, "setFlashModeInternal : " + ex);
        }
    }
