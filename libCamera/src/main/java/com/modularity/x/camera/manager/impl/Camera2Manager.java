...
public class Camera2Manager extends BaseCameraManager<String> implements ImageReader.OnImageAvailableListener {
        ...
        private void processCaptureResultInternal(@NonNull CaptureResult result, @CameraState int cameraPreviewState) {
            switch (cameraPreviewState) {
                ...
                case CameraState.STATE_WAITING_LOCK:
                    ...
                    break;
                case CameraState.STATE_WAITING_PRE_CAPTURE:
                    Integer aeState = result.get(CaptureResult.CONTROL_AE_STATE);
                    if (aeState == null ||
                    ...
                    break;
                case CameraState.STATE_WAITING_NON_PRE_CAPTURE:
    ...
    private ImageReader.OnImageAvailableListener onPreviewImageAvailableListener = new ImageReader.OnImageAvailableListener() {
        ...
        public void onImageAvailable(ImageReader reader) {
            try (Image image = reader.acquireNextImage()) {
                ...
                LogManager.iTag(TAG, "error for image preview : " + ex);
            }
        }
    };
                ...
                try {
                    ...
                    cameraManager.openCamera(currentCameraId, new CameraDevice.StateCallback() {
                        ...
                        public void onOpened(@NonNull CameraDevice camera) {
                            ...
                            notifyCameraOpened();
                            LogManager.iTag(TAG, "Camera opened cost : "
                        ...
                        public void onDisconnected(@NonNull CameraDevice camera) {
                            LogManager.iTag(TAG, "Camera disconnected.");
                            camera.close();
                            cameraDevice = null;
                        ...
                        public void onError(@NonNull CameraDevice camera, int error) {
                            LogManager.iTag(TAG, "Camera open error : " + error);
                            camera.close();
                            cameraDevice = null;
    ...
    public void setMediaType(int mediaType) {
        LogManager.iTag(TAG, "setMediaType : " + mediaType + " with mediaType " + this.mediaType);
            ...
            backgroundHandler.post(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        LogManager.iTag(TAG, "setMediaType : " + ex);
                    }
                }
            });
        }
        ...
        if (isCameraOpened() && previewRequestBuilder != null) {
            ...
            if (captureSession != null) {
                try {
                    ...
                    LogManager.iTag(TAG, "setAutoFocus error : " + e);
                }
            } else {
                LogManager.iTag(TAG, "setAutoFocus captureSession is null.");
            }
        } else {
            LogManager.iTag(TAG, "setAutoFocus camera not open or previewRequestBuilder is null");
        }
    }
            ...
            if (succeed) {
                ...
                if (captureSession != null) {
                    try {
                        ...
                        LogManager.iTag(TAG, "setFlashMode error : " + e);
                    }
                } else {
                    LogManager.iTag(TAG, "setFlashMode captureSession is null.");
                }
            } else {
                LogManager.iTag(TAG, "setFlashMode failed.");
            }
        } else {
            LogManager.iTag(TAG, "setFlashMode camera not open or previewRequestBuilder is null");
        }
    }
            ...
            if (succeed) {
                ...
                if (captureSession != null) {
                    try {
                        ...
                        LogManager.iTag(TAG, "setZoom error : " + e);
                    }
                } else {
                    LogManager.iTag(TAG, "setZoom captureSession is null.");
                }
            } else {
                LogManager.iTag(TAG, "setZoom failed : setZoomInternal failed.");
            }
        } else {
            LogManager.iTag(TAG, "setZoom failed : camera not open.");
        }
    }
            ...
            backgroundHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (prepareVideoRecorder()) {
                        ...
                        LogManager.iTag(TAG, "startVideoRecord : failed when prepare video recorder.");
...
//        releaseTexture();
    ...
    public void onImageAvailable(ImageReader reader) {
        try {
            try (Image image = reader.acquireNextImage()) {
                ...
                LogManager.iTag(TAG, "onImageAvailable error" + e);
    ...
    private void initCameraInfo(Context context) {
        ...
        try {
            ...
            LogManager.iTag(TAG, "initCameraInfo error " + e);
        }
        LogManager.iTag(TAG, "initCameraInfo basic cost : " + (System.currentTimeMillis() - start) + " ms");
    ...
    private void prepareCameraOutputs() {
        ...
        try {
            ...
            LogManager.iTag(TAG, "initCameraInfo error " + ex);
            notifyCameraOpenError(new RuntimeException(ex));
        }
        LogManager.iTag(TAG, "initCameraInfo get map cost : " + (System.currentTimeMillis() - start) + " ms");
        ...
        LogManager.iTag(TAG, "prepareCameraOutputs cost : " + (System.currentTimeMillis() - start) + " ms");
    }
    ...
    private void adjustCameraConfiguration(boolean forceCalculate) {
        ...
        LogManager.iTag(TAG, "previewSize: " + previewSize + " oldPreviewSize:" + oldPreviewSize);
                        ...
                        cameraDevice.createCaptureSession(Arrays.asList(workingSurface, imageReader.getSurface(), previewReader.getSurface()),
                                new CameraCaptureSession.StateCallback() {
                                    ...
                                    public void onConfigured(@NonNull CameraCaptureSession cameraCaptureSession) {
                                        if (isCameraOpened()) {
                                            ...
                                            try {
                                                ...
                                                LogManager.iTag(TAG, "create preview session error " + ex);
                                                notifyCameraOpenError(ex);
                                    ...
                                    public void onConfigureFailed(@NonNull CameraCaptureSession cameraCaptureSession) {
                                        LogManager.iTag(TAG, "onConfigureFailed");
                                        notifyCameraOpenError(new Throwable("Camera capture session configure failed."));
                                    }
                                }, backgroundHandler);
                    } catch (Exception ex) {
                        LogManager.iTag(TAG, "createPreviewSession error " + ex);
                        notifyCameraOpenError(ex);
                    }
                }
            };
            ...
            try {
                ...
                captureSession.capture(captureBuilder.build(), new CameraCaptureSession.CaptureCallback() {
                    ...
                    public void onCaptureCompleted(@NonNull CameraCaptureSession session,
                        ...
                        LogManager.iTag(TAG, "onCaptureCompleted: ");
                    }
                }, null);
            } catch (CameraAccessException e) {
                LogManager.iTag(TAG, "Error during capturing picture");
                notifyCameraCaptureFailed(e);
            }
        } else {
        ...
        final Runnable sessionCreationTask = new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    cameraDevice.createCaptureSession(surfaces, new CameraCaptureSession.StateCallback() {
                        ...
                        public void onConfigured(@NonNull CameraCaptureSession cameraCaptureSession) {
                            ...
                            try {
                                ...
                                LogManager.iTag(TAG, "videoRecorder.start(): " + e);
                    ...
                    LogManager.iTag(TAG, "startVideoRecord: " + e);
                    notifyVideoRecordError(e);
                }
            }
        };
    ...
    private void runPreCaptureSequence() {
        try {
            ...
            LogManager.iTag(TAG, "runPreCaptureSequence error " + e);
        }
    }
    ...
    private boolean setFlashModeInternal() {
        try {
            ...
            if (isFlashAvailable == null || !isFlashAvailable) {
                LogManager.iTag(TAG, "Flash is not available.");
                return false;
            }
            switch (flashMode) {
            ...
            LogManager.iTag(TAG, "setFlashMode error : " + ex);
        }
        return false;
    }
    ...
    private void lockFocus() {
        try {
            ...
            LogManager.iTag(TAG, "lockFocus : error during focus locking");
        }
    }
    ...
    private void unlockFocus() {
        try {
            ...
            LogManager.iTag(TAG, "unlockFocus : error during focus unlocking");
        }
    }
    ...
    private boolean prepareVideoRecorder() {
        ...
        try {
            ...
            LogManager.iTag(TAG, "IllegalStateException preparing MediaRecorder: " + error.getMessage());
            notifyVideoRecordError(error);
        } catch (IOException error) {
            LogManager.iTag(TAG, "IOException preparing MediaRecorder: " + error.getMessage());
            notifyVideoRecordError(error);
        } catch (Exception error) {
            LogManager.iTag(TAG, "Error during preparing MediaRecorder: " + error.getMessage());
            notifyVideoRecordError(error);
        }
    ...
    private void closePreviewSession() {
        if (captureSession != null) {
            ...
            try {
                ...
                LogManager.iTag(TAG, "closePreviewSession error : " + e);
            } finally {
                captureSession = null;
            }
        }
    ...
    abstract static class CaptureSessionCallback extends CameraCaptureSession.CaptureCallback {
        ...
        @IntDef({CameraState.STATE_PREVIEW, CameraState.STATE_WAITING_LOCK, CameraState.STATE_WAITING_PRE_CAPTURE,
